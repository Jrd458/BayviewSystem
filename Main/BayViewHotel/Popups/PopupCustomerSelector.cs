...
namespace BayViewHotel.Popups
{
    ...
    {
        private AddBooking _master; // Allows us to call functions/set variables in booking form
        ...
        private void dataGridCustomer_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            _master._selectedCustomerId = Convert.ToInt32(customerId); // Sets the customer ID in booking screen
            _master.SetCustomerSelectionLabel(Convert.ToInt32(customerId)); // Uses customer ID to grab first and last name of customer which is displayed in the label
            this.Close();
        }
    }
}
